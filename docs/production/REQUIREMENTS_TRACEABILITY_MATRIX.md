...
| RTM-STATE-02 | FDA606 | 所有狀態寫入經單一命令服務，無路由直寫 | All SOP | `bloodUnitCommands.ts:executeBloodUnitTransition` | `h1DirectWrites.test.ts`, **`noDirectStatusWrites.test.ts`（靜態掃描棘輪防線）** | 行為測試 + **靜態回歸防線已建**：掃所有路由的直接狀態寫入，違規集合須等於已審 allowlist（4 處：inventory 非轉換、2× lab-tests 已 guard、reconciliation 修復），新增直寫即測試失敗 | ✅ |
...
| RTM-TRACE-01 | FDA606, ISBT128 | 捐血者↔受血者雙向完整追溯報表 | SOP9 | **`services/traceability.ts`（forward 捐血者→受血者 / backward 血袋→捐血者）, `app/api/v1/trace/route.ts`（RBAC 限品質/HV 角色）** | `traceability.test.ts`(5), `traceRoute.test.ts`(4) | 雙向追溯報表已建（含 IDM 狀態、配發/輸血/配血受血者去重、不良反應連動）+ 路由整合測試 + 前端面板（HemovigilanceView，三語）；待 UAT 實資料 + reviewer 簽核 | 🟠 |
