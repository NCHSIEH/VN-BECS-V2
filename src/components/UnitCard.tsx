    ...
    <div className="clinical-card p-5 flex flex-col gap-4 group transition-all hover:scale-[1.02] cursor-pointer bg-transparent">
      ...
      <div className="flex items-center gap-3">
        <div className="w-14 h-14 bg-transparent rounded-2xl flex flex-col items-center justify-center border border-slate-100 shadow-inner">
          ...
          <span className="text-[8px] font-bold text-slate-400 mt-1">{rhd === 'Positive' ? 'RhD+' : 'RhD-'}</span>
        </div>
        <div className="flex-1 grid grid-cols-2 gap-3">
          <div className="flex flex-col">
            ...
            <span className="text-xs font-bold text-slate-500">{new Date(expiry).toLocaleDateString()}</span>
          </div>
          {temp !== undefined && (
            <div className="flex flex-col">
              <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest mb-1 flex items-center gap-1">
