    ...
    <div className="clinical-card p-5 flex flex-col gap-4 group transition-all hover:scale-[1.02] cursor-pointer bg-transparent">
      <div className="flex justify-between items-start">
        <div className="flex flex-col">
          <div className="flex items-center gap-2 text-clinical-muted mb-1">
            <Barcode size={14} />
      ...
      <div className="flex items-center gap-3">
        <div className="w-14 h-14 bg-transparent rounded-2xl flex flex-col items-center justify-center border border-clinical-border shadow-inner">
          <span className="text-xl font-black text-clinical-primary leading-none">{abo}</span>
          <span className="text-[8px] font-bold text-clinical-muted mt-1">{rhd === 'Positive' ? 'RhD+' : 'RhD-'}</span>
        </div>
        <div className="flex-1 grid grid-cols-2 gap-3">
          <div className="flex flex-col">
            <span className="text-[8px] font-black text-clinical-text uppercase tracking-widest mb-1 flex items-center gap-1">
              <Calendar size={10} /> Expiry
            </span>
            <span className="text-xs font-bold text-clinical-muted">{new Date(expiry).toLocaleDateString()}</span>
          </div>
          {temp !== undefined && (
            <div className="flex flex-col">
              <span className="text-[8px] font-black text-clinical-text uppercase tracking-widest mb-1 flex items-center gap-1">
      ...
      <div className="pt-2 border-t border-slate-50 flex justify-between items-center">
        <div className="h-1.5 flex-1 bg-clinical-bg rounded-full overflow-hidden mr-4">
