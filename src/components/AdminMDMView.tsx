...
export function AdminMDMView({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0b1120] text-slate-200">
      <header className="p-4 border-b border-indigo-900/50 bg-[#020617] flex items-center justify-between shadow-lg">
         <div className="flex items-center gap-4">
           <button onClick={onBack} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 transition-colors">
              <ArrowLeft size={18} />
           </button>
           <div className="flex items-center gap-3">
              ...
              <div>
                 <h1 className="font-bold text-indigo-400 uppercase tracking-widest text-sm">{t('mdm_title')}</h1>
                 <p className="text-[10px] text-slate-500 uppercase tracking-wide">{t('mdm_subtitle')}</p>
              </div>
           </div>
         </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden p-6 gap-6">
         ...
         <div className="w-64 flex flex-col gap-2">
            <button onClick={() => setTab('Orgs')} className={`p-4 rounded-xl flex items-center gap-3 transition-all ${tab === 'Orgs' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-900/50 text-slate-500 hover:bg-slate-800'}`}>
               <Building size={18} /> <span className="text-xs font-bold uppercase tracking-widest">{t('mdm_orgs')}</span>
            </button>
            <button onClick={() => setTab('MSD')} className={`p-4 rounded-xl flex items-center gap-3 transition-all ${tab === 'MSD' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-900/50 text-slate-500 hover:bg-slate-800'}`}>
               <Users size={18} /> <span className="text-xs font-bold uppercase tracking-widest">{t('mdm_msd')}</span>
            </button>
            <button onClick={() => setTab('Catalog')} className={`p-4 rounded-xl flex items-center gap-3 transition-all ${tab === 'Catalog' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-900/50 text-slate-500 hover:bg-slate-800'}`}>
         ...
         <div className="flex-1 bg-[#020617] border border-slate-800 rounded-2xl p-6 overflow-y-auto">
            {tab === 'Orgs' && (
              <div>
                <h2 className="text-xl font-bold text-slate-200 mb-6 flex items-center gap-2 border-b border-slate-800 pb-4">
                  <Building className="text-slate-500" /> {t('mdm_org_dir')}
                </h2>
                ...
                <form onSubmit={handleAddOrg} className="grid grid-cols-4 gap-4 mb-8 bg-slate-900/50 p-4 rounded-xl border border-slate-800/50">
                  ...
                  <button type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 rounded flex justify-center items-center gap-1 transition-colors">
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {orgs.map(o => (
                        <tr key={o.id} className="hover:bg-slate-900/50 transition-colors">
                          <td className="p-3 font-mono text-xs text-slate-500">{o.id}</td>
                          <td className="p-3 font-bold text-slate-300">{o.name}</td>
                          <td className="p-3">
                             <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-800 text-slate-400">{o.type}</span>
                          </td>
                          <td className="p-3 text-slate-400">{o.location}</td>
                          <td className="p-3 text-right">
                             <button onClick={() => setEditingOrg(o)} className="p-2 text-slate-500 hover:text-white transition-colors">
                                <Edit2 size={16} />
                             </button>
                          </td>
                        </tr>
                  ...
                  <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
                    <form onSubmit={handleUpdateOrg} className="clinical-card max-w-lg w-full bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
                       <div className="p-6 border-b border-slate-800 bg-slate-950 flex justify-between items-center">
                          <h3 className="text-sm font-black text-white uppercase italic tracking-widest">{t('mdm_edit_org')}</h3>
                          <button type="button" onClick={() => setEditingOrg(null)} className="p-2 hover:bg-slate-800 rounded-lg text-slate-500 hover:text-white transition-colors"><X size={20} /></button>
                       </div>
                       <div className="p-8 space-y-6">
                          <div className="flex flex-col gap-2">
                             <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Name</label>
                             <input value={editingOrg.name} onChange={e=>setEditingOrg({...editingOrg, name: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all" />
                          </div>
                          <div className="grid grid-cols-2 gap-6">
                             <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Type</label>
                                <select value={editingOrg.type} onChange={e=>setEditingOrg({...editingOrg, type: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all">
                                   <option>Hospital</option>
                                   <option>BloodCenter</option>
                                   <option>Hub</option>
                                </select>
                             ...
                             <div className="flex flex-col gap-2">
                                ...
                                <input value={editingOrg.location} onChange={e=>setEditingOrg({...editingOrg, location: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all" />
                             </div>
                          </div>
                       </div>
                       <div className="p-6 bg-slate-950/50 border-t border-slate-800 flex justify-end gap-3">
                          <button type="button" onClick={() => setEditingOrg(null)} className="px-6 py-2 text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest transition-colors">{t('mdm_cancel')}</button>
            ...
            {tab === 'MSD' && (
              <div>
                <h2 className="text-xl font-bold text-slate-200 mb-6 flex items-center gap-2 border-b border-slate-800 pb-4">
                  <Users className="text-slate-500" /> {t('mdm_msd_tactical')}
                </h2>
                        ...
                        <tr key={u.id} className={`hover:bg-slate-900/50 transition-colors ${!u.isActive ? 'opacity-50' : ''}`}>
                          <td className="p-3 font-bold text-slate-300">
                            <div className="flex items-center gap-3">
                               ...
                               <div>
                                  ...
                                  <p className="text-[9px] text-slate-500 font-mono">{u.id}</p>
                          ...
                          <td className="p-3">
                             <div className="flex flex-col gap-1">
                                <span className="px-2 py-0.5 w-fit rounded text-[10px] font-bold uppercase tracking-wider bg-indigo-950/50 text-indigo-400 border border-indigo-900/30">{u.role}</span>
                                <span className="text-[10px] text-slate-400">{u.orgName}</span>
                             </div>
                          </td>
                          <td className="p-3">
                          ...
                          <td className="p-3 text-right flex items-center justify-end gap-2">
                             <button onClick={() => setEditingUser(u)} className="p-2 text-slate-500 hover:text-white transition-colors">
                                <Edit2 size={16} />
                             </button>
                ...
                {editingUser && (
                  <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
                    <form onSubmit={handleUpdateUser} className="clinical-card max-w-lg w-full bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
                       <div className="p-6 border-b border-slate-800 bg-slate-950 flex justify-between items-center">
                          <h3 className="text-sm font-black text-white uppercase italic tracking-widest">{t('mdm_edit_user')}</h3>
                          <button type="button" onClick={() => setEditingUser(null)} className="p-2 hover:bg-slate-800 rounded-lg text-slate-500 hover:text-white transition-colors"><X size={20} /></button>
                       </div>
                       <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                           <div className="grid grid-cols-2 gap-6">
                              <div className="flex flex-col gap-2">
                                 <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Username</label>
                                 <input value={editingUser.username} onChange={e=>setEditingUser({...editingUser, username: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all" />
                              </div>
                              <div className="flex flex-col gap-2">
                                 <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Password</label>
                                 <input type="text" value={editingUser.password || ''} onChange={e=>setEditingUser({...editingUser, password: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all font-mono" placeholder="New Password" />
                              </div>
                           </div>
                          <div className="grid grid-cols-2 gap-6">
                             <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Role</label>
                                <select value={editingUser.role} onChange={e=>setEditingUser({...editingUser, role: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all">
                                   <option>HospitalOperator</option>
                                   <option>Nurse</option>
                                   <option>WarehouseIssuer</option>
                                   <option>Dispatcher</option>
                             ...
                             <div className="flex flex-col gap-2">
                                ...
                                <select value={editingUser.orgId} onChange={e=>setEditingUser({...editingUser, orgId: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all">
                                   {orgs.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
                                </select>
                             </div>
                          </div>
                          <div className="grid grid-cols-2 gap-6">
                             <div className="flex flex-col gap-2">
                                ...
                                <input value={editingUser.details?.email || ''} onChange={e=>setEditingUser({...editingUser, details: {...editingUser.details, email: e.target.value}})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all" />
                             </div>
                             <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Phone</label>
                                <input value={editingUser.details?.phone || ''} onChange={e=>setEditingUser({...editingUser, details: {...editingUser.details, phone: e.target.value}})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-rose-500 transition-all" />
                             </div>
                          </div>
                       </div>
                       <div className="p-6 bg-slate-950/50 border-t border-slate-800 flex justify-end gap-3">
                          <button type="button" onClick={() => setEditingUser(null)} className="px-6 py-2 text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest transition-colors">{t('mdm_cancel')}</button>
            ...
            {tab === 'Catalog' && (
              <div>
                <h2 className="text-xl font-bold text-slate-200 mb-6 flex items-center gap-2 border-b border-slate-800 pb-4">
                  <Shield className="text-slate-500" /> {t('mdm_catalog_isbt')}
                </h2>
                ...
                <form onSubmit={handleAddProduct} className="grid grid-cols-6 gap-4 mb-8 bg-slate-900/50 p-4 rounded-xl border border-slate-800/50">
                  ...
                  <div className="flex flex-col gap-1 justify-center">
                    <label className="flex items-center gap-2 text-[10px] text-slate-400 uppercase font-bold cursor-pointer">
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {products.map(p => (
                        <tr key={p.productCode} className="hover:bg-slate-900/50 transition-colors">
                          <td className="p-3 font-mono text-indigo-400 font-bold">{p.productCode}</td>
                          <td className="p-3 text-slate-300">{p.alias}</td>
