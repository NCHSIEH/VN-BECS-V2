...
export function CourierView() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-full overflow-y-auto lg:overflow-hidden p-4 lg:p-0 animate-in fade-in duration-500">
          ...
          {orders.map(order => (
            <div 
              ...
              className={`p-4 rounded-2xl border transition-all group ${
                selectedOrder?.id === order.id ? 'bg-clinical-card border-sky-500 shadow-xl shadow-sky-900/20' : 'bg-clinical-bg border-clinical-border hover:border-clinical-border'
              }`}
            >
              <div className="flex justify-between items-center mb-3">
                <span className="font-mono text-[10px] font-black text-clinical-muted tracking-widest">{order.id}</span>
      ...
      <div className="flex-1 bg-clinical-bg border border-clinical-border rounded-2xl p-8 flex flex-col shadow-2xl relative overflow-hidden">
        {!selectedOrder ? (
          ...
          <div className="flex flex-col h-full mx-auto w-full max-w-3xl">
             ...
             <div className="grid grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-clinical-card/20 border border-clinical-border p-6 rounded-3xl flex flex-col justify-center col-span-2">
                   ...
                   <div className="flex items-center gap-4">
                      <div className="flex-1 h-2 bg-clinical-bg rounded-full overflow-hidden p-0.5 border border-clinical-border shadow-inner">
                         <div 
             ...
             {/* Action Bar */}
             <div className="mt-auto pt-8 border-t border-clinical-border">
               {isWasted ? (
                  <button 
      ...
      {showHandoverModal && (
        <div className="fixed inset-0 bg-clinical-bg/95 backdrop-blur-3xl z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="clinical-card bg-clinical-card border-emerald-500/30 w-full max-w-sm p-10 shadow-[0_0_150px_rgba(16,185,129,0.15)] rounded-[40px]">
             <div className="text-center mb-10">
                ...
                <p className="text-clinical-muted text-[10px] font-black uppercase tracking-widest mt-2">Hospital Recipient PIN Required</p>
             </div>
             ...
             <div className="space-y-8">
                <div>
                   <input 
                     ...
                     className="w-full bg-clinical-bg border-2 border-clinical-border text-emerald-400 text-center text-5xl font-black tracking-[0.5em] py-8 rounded-3xl focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all shadow-inner placeholder:opacity-10"
                     placeholder="----"
                   />
                </div>
                ...
                <div className="flex flex-col gap-4">
                   ...
                   <button onClick={() => setShowHandoverModal(false)} className="w-full py-4 text-[9px] font-black text-clinical-muted uppercase tracking-widest hover:text-white transition-all">Cancel Operation</button>
                </div>
             </div>
          </div>
        </div>
