    ...
    <header className="h-28 border-b border-slate-800/50 bg-[#020617]/80 backdrop-blur-2xl z-[60] sticky top-0 flex flex-col shadow-2xl">
      ...
      <div className="flex-1 flex items-center justify-between px-8 lg:px-12">
        <div className="flex items-center gap-8">
           <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                 <h1 className="text-sm lg:text-base font-black text-white uppercase tracking-[0.2em] italic leading-tight">
                    {systemName}
                 </h1>
                 <div className="flex items-center gap-2 mt-1">
                    <Globe size={12} className="text-rose-500" />
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">{t('ui_republic_vn')}</span>
                 </div>
              </div>
           </div>
           ...
           <nav className="hidden xl:flex items-center gap-6 border-l border-slate-800 ml-8 pl-8">
              {[ { id: 'live', label: t('ui_live_overview') }, { id: 'logistics', label: t('ui_logistics') }, { id: 'analytics', label: t('ui_analytics') }, { id: 'predictions', label: t('ui_predictions') }, { id: 'reports', label: t('ui_reports') } ].map(item => (
                <button key={item.id} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest transition-colors relative group">
                  {item.label}
        ...
        <div className="flex items-center gap-6 lg:gap-8">
           {/* Search */}
           <div className="hidden lg:flex items-center gap-3 px-4 py-2 bg-slate-900/50 border border-slate-800 rounded-xl text-slate-500 focus-within:border-rose-500 transition-all">
              <Search size={16} />
           ...
           {onOpenSwitcher && (
             <button 
               ...
               className="hidden lg:flex items-center gap-3 px-6 py-2.5 bg-gradient-to-r from-rose-900/40 to-indigo-900/40 border border-rose-500/30 text-rose-500 rounded-2xl hover:from-rose-600 hover:to-indigo-600 hover:text-white transition-all group shadow-lg shadow-rose-950/20 active:scale-95"
             >
           ...
           <div className="flex items-center gap-4 border-r border-slate-800 pr-8">
              <button onClick={onOpenMessages} className="relative p-3 bg-slate-900/50 hover:bg-slate-800 rounded-xl text-slate-400 hover:text-rose-500 transition-all border border-slate-800">
                 <Mail size={20} />
                 {unreadMessages > 0 && (
              ...
              <button className="relative p-3 bg-slate-900/50 hover:bg-slate-800 rounded-xl text-slate-400 hover:text-rose-500 transition-all border border-slate-800">
                 <Bell size={20} />
           ...
           <div className="relative" ref={menuRef}>
              ...
              >
                 <div className="flex flex-col text-right hidden sm:block">
                    <p className="text-[11px] font-black text-white uppercase italic leading-none mb-1">{user.username}</p>
                 ...
                 <ChevronDown size={16} className={`text-slate-600 group-hover:text-white transition-all ${showProfileMenu ? 'rotate-180' : ''}`} />
              </div>
                ...
                {showProfileMenu && (
                  ...
                  >
                     <div className="px-6 py-4 border-b border-slate-800 bg-slate-950/50">
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('ui_command_identity')}</p>
                        <p className="text-xs font-black text-white uppercase italic">{user.username}</p>
                     </div>
                     <div className="p-2">
                        <button className="w-full flex items-center gap-4 px-4 py-3 rounded-2xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all group">
                        ...
                        <button 
                          ...
                          className="w-full flex items-center gap-4 px-4 py-3 rounded-2xl text-rose-500 hover:text-white hover:bg-rose-600 transition-all group"
                        >
                           <LogOut size={18} />
      ...
      <div className="h-8 bg-slate-950/50 flex items-center justify-between px-12 border-t border-slate-800/30">
        <div className="flex items-center gap-4">
