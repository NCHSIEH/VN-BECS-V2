...
export function LimsFlowIndicator({ activeTab }: { activeTab: string }) {
  ...
  return (
    <div className="bg-clinical-bg/80 border-b border-clinical-border p-3 overflow-x-auto custom-scrollbar">
      <div className="flex items-center gap-2 text-xs font-bold w-max mx-auto px-4">
        <span className="text-clinical-muted mr-2 uppercase tracking-widest hidden lg:inline">{t('flow_lims_title')}</span>
        {limsSteps.map((step, idx) => (
          <React.Fragment key={step.id}>
            <div className={`px-4 py-1.5 rounded-full flex flex-col items-center justify-center transition-all ${
              activeTab === step.id 
                ? 'bg-blue-400 border border-blue-950/300 shadow-[0_0_10px_rgba(59,130,246,0.3)] text-clinical-text'
                : 'bg-clinical-bg/50 text-clinical-muted border border-clinical-border'
            }`}>
            ...
            {idx < limsSteps.length - 1 && (
              <div className="text-clinical-muted font-black">➔</div>
            )}
          </React.Fragment>
        ))}
      </div>
...
export function FlowIndicator({ role }: { role: Role }) {
  ...
  return (
    <div className="bg-clinical-bg/80 border-b border-clinical-border p-3 overflow-x-auto custom-scrollbar shadow-inner">
      <div className="flex items-center gap-2 text-xs font-bold w-max mx-auto px-4">
        <span className="text-clinical-muted mr-2 uppercase tracking-widest hidden xl:inline">{t('flow_sc_title')}</span>
        {mainSteps.map((step, idx) => (
          <React.Fragment key={idx}>
            <div className={`px-4 py-1.5 rounded-full flex flex-col items-center justify-center transition-all whitespace-nowrap ${
              currentStepIdx === idx 
                ? 'bg-lime-400 border border-lime-400 shadow-[0_0_10px_rgba(16,185,129,0.3)] text-clinical-text font-extrabold'
                : currentStepIdx > idx 
            ...
            {idx < mainSteps.length - 1 && (
              <div className={`text-sm ${currentStepIdx > idx ? 'text-lime-950/300 font-bold' : 'text-clinical-muted'}`}>➔</div>
            )}
          </React.Fragment>
        ))}
      </div>
