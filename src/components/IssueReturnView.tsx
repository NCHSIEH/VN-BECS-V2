  ...
  return (
    <div className="flex flex-col h-full gap-4 max-w-6xl mx-auto w-full p-4 overflow-y-auto">
      <div className="grid grid-cols-12 gap-4">
        {/* Issue Form */}
        <div className="col-span-12 lg:col-span-5 bg-clinical-bg border border-clinical-border rounded-xl p-5 flex flex-col">
          <h2 className="text-sm font-bold text-clinical-muted uppercase tracking-widest border-b border-clinical-border pb-2 mb-4 flex items-center gap-2">
          ...
          <form onSubmit={handleIssue} className="flex flex-col gap-4 flex-1">
            <div>
              <label className="block text-xs font-bold text-clinical-muted uppercase mb-1">{t('iss_form_unit')}</label>
              <input required value={componentId} onChange={e => setComponentId(e.target.value)}
                placeholder="Scan barcode..." className="w-full bg-clinical-bg border border-clinical-border rounded-lg px-4 py-2 text-clinical-text font-mono" />
            </div>
            <div>
              <label className="block text-xs font-bold text-clinical-muted uppercase mb-1">{t('iss_form_patient')}</label>
              <input required value={patientId} onChange={e => setPatientId(e.target.value)}
                placeholder="MRN-XXXX" className="w-full bg-clinical-bg border border-clinical-border rounded-lg px-4 py-2 text-clinical-text" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-clinical-muted uppercase mb-1">{t('iss_form_ward')}</label>
                <input required value={issuedTo} onChange={e => setIssuedTo(e.target.value)}
                  placeholder="e.g. ICU-3" className="w-full bg-clinical-bg border border-clinical-border rounded-lg px-4 py-2 text-clinical-text" />
              </div>
              <div>
                <label className="block text-xs font-bold text-clinical-muted uppercase mb-1">{t('mdm_user_name')}</label>
                <input required value={issuedBy} onChange={e => setIssuedBy(e.target.value)}
                  placeholder="Staff name" className="w-full bg-clinical-bg border border-clinical-border rounded-lg px-4 py-2 text-clinical-text" />
              </div>
            </div>
            <button type="submit" className="mt-auto bg-cyan-600 hover:bg-cyan-500 text-clinical-text font-bold py-3 rounded-xl transition shadow-lg">
              {t('iss_issue_btn')}
            </button>
          </form>
        </div>
        ...
        <div className="col-span-12 lg:col-span-7 bg-clinical-bg border border-clinical-border rounded-xl p-5 flex flex-col">
          <h2 className="text-sm font-bold text-clinical-muted uppercase tracking-widest border-b border-clinical-border pb-2 mb-4 flex justify-between">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
            {activeRecords.map(r => {
              ...
              return (
                <div key={r.id} className={`p-3 rounded-lg border ${overTime ? 'border-rose-900/50 bg-rose-950/10 animate-pulse' : warning ? 'border-amber-900/50 bg-amber-950/10' : 'border-clinical-border bg-clinical-bg'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <span className="text-sm font-mono text-clinical-text font-bold">{r.componentId}</span>
                      <span className="text-xs text-clinical-muted ml-2">→ {r.patientId}</span>
                    </div>
                  ...
                  <div className="text-xs text-clinical-muted flex justify-between">
                    ...
                    {returnId === r.id ? (
                      <div className="flex gap-2 items-center">
                        ...
                        <button onClick={() => handleReturn(r.id)} className="text-[10px] bg-amber-600 text-clinical-text px-2 py-0.5 rounded font-bold">{t('lims_form_submit')}</button>
                      </div>
                    ) : (
                      <button onClick={() => { setReturnId(r.id); setColdChainOk(true); setVisualOk(true); }}
                        className="text-[10px] bg-clinical-bg border border-amber-800 text-amber-400 px-2 py-0.5 rounded font-bold hover:bg-amber-900/30 transition">
            ...
            {activeRecords.length === 0 && (
              <div className="text-clinical-muted text-sm text-center py-8">No active issues.</div>
            )}

            {/* Completed/Returned Records */}
            {records.filter(r => r.returnedAt || r.returnStatus === 'WASTED').length > 0 && (
              <>
                <div className="border-t border-clinical-border mt-4 pt-3">
                  <h3 className="text-xs font-bold text-clinical-muted uppercase mb-2">History</h3>
                </div>
                {records.filter(r => r.returnedAt || r.returnStatus === 'WASTED').map(r => (
                  <div key={r.id} className="p-2 rounded border border-clinical-border bg-clinical-bg flex justify-between items-center text-xs text-clinical-muted">
