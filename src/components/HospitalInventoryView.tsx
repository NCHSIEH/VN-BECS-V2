    ...
    <div className="flex-1 flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
         {filteredHospitals.map(hospital => (
            <div key={hospital.id} className="clinical-card group hover:border-slate-600 transition-all bg-clinical-card/10 backdrop-blur-xl border-clinical-border overflow-hidden flex flex-col">
               <div className="p-8 border-b border-clinical-border flex justify-between items-start bg-clinical-card/20">
                  <div className="flex items-center gap-5">
                     <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-clinical-text shadow-lg ${
                     ...
                     <div>
                        <h3 className="text-2xl font-black text-clinical-text tracking-tighter uppercase italic">{hospital.name}</h3>
                        <div className="flex items-center gap-3 mt-1">
                           <span className="flex items-center gap-1 text-[10px] font-black text-clinical-muted uppercase tracking-widest"><MapPin size={10} /> {hospital.location}</span>
                           <span className="text-clinical-text">•</span>
                           <span className="text-[10px] font-mono font-bold text-clinical-muted">{hospital.id}</span>
                        </div>
                     </div>
                  </div>
               ...
               <div className="p-8 grid grid-cols-2 md:grid-cols-4 gap-6 bg-clinical-bg/20">
                  {Object.entries(hospital.stock).map(([type, qty]: [string, any]) => {
                     ...
                     return (
                        <div key={type} className={`p-4 rounded-2xl border ${cls.border} ${cls.bg} flex flex-col items-center justify-center gap-1 transition-all hover:scale-105`}>
                           ...
                           <span className="text-[8px] font-black text-clinical-muted uppercase tracking-widest">{dos.toFixed(1)} DOS</span>
                        </div>
                     );
                  })}
               </div>
