...
export function DocsView({ onBack }: { onBack: () => void }) {
            ...
            <div className="flex flex-col gap-4">
              {section.items?.map((item, i) => (
                 <div key={i} className="flex gap-4 items-center bg-[#0b1120] p-3 rounded-lg border border-slate-800">
                    ...
                    <div className="text-sm text-slate-300 font-mono leading-relaxed">{item}</div>
                 </div>
              ))}
            </div>
          </div>
          ...
          <div key={idx} className="mb-6">
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-800">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-900 text-slate-400 uppercase tracking-widest text-[10px]">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800 bg-[#0b1120]">
                  {section.tableData?.rows.map((row, i) => (
                    <tr key={i} className="hover:bg-slate-900/50 transition-colors">
                      {row.map((cell, j) => (
                        <td key={j} className={`p-4 ${j === 0 ? 'font-bold text-cyan-400' : 'text-slate-400'}`}>{cell}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0b1120] text-slate-200 overflow-hidden font-sans">
      <header className="p-4 border-b border-indigo-900/50 bg-[#020617] flex items-center justify-between shadow-lg shrink-0">
         <div className="flex items-center gap-4">
           ...
           <div className="flex items-center gap-3">
              ...
              <div>
                 <h1 className="font-bold text-slate-200 uppercase tracking-widest text-sm">{t('doc_title')}</h1>
                 <p className="text-[10px] text-slate-500 uppercase tracking-wide">{t('doc_subtitle')}</p>
              </div>
           </div>
         </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-72 bg-[#020617] border-r border-slate-800 flex flex-col p-4 overflow-y-auto shrink-0 shadow-[4px_0_24px_rgba(0,0,0,0.5)] z-10">
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4 px-2">Table of Contents</div>
          <div className="flex flex-col gap-2">
            {menuItems.map(item => (
              <button 
                key={item.id}
                onClick={() => setActiveDoc(item.id)}
                className={`flex items-center gap-3 p-3.5 rounded-xl text-sm transition-all text-left group ${activeDoc === item.id ? 'bg-indigo-900/40 text-indigo-400 border border-indigo-800/50 font-bold shadow-lg' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200 border border-transparent'}`}
              >
                <div className={`${activeDoc === item.id ? 'text-indigo-400' : 'text-slate-500 group-hover:text-slate-300'}`}>
                  {item.icon}
                </div>
        ...
        <div className="flex-1 p-6 lg:p-12 overflow-y-auto bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-900/10 via-[#0b1120] to-[#0b1120]">
          ...
          <div className="max-w-4xl mx-auto mb-8 bg-gradient-to-r from-cyan-900/40 to-indigo-900/40 border border-cyan-800/50 rounded-2xl p-6 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-400 text-sm">{lang === 'zh-TW' ? '我們準備了萬字級的圖文手把手外部手冊，非常適合初次接觸系統的新手。' : lang === 'vi' ? 'Chúng tôi đã chuẩn bị một sổ tay bên ngoài với hàng ngàn từ và hình ảnh.' : 'We have prepared an extensive external manual with step-by-step guides.'}</p>
            </div>
            <a 
              href={`/docs/manual_${lang === 'zh-TW' ? 'zh' : lang}.html`} 
              target="_blank" 
              rel="noopener noreferrer"
              className="flex items-center gap-2 px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl transition-all shadow-[0_0_15px_rgba(8,145,178,0.5)] shrink-0"
            >
          ...
          <div className="max-w-4xl mx-auto bg-[#020617]/80 backdrop-blur-md border border-slate-800 rounded-3xl p-8 lg:p-12 shadow-2xl">
             <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
               <div className="mb-10 pb-6 border-b border-slate-800">
                 <h2 className="text-3xl font-black text-slate-200 mb-4 tracking-tight">{moduleData.title}</h2>
                 <p className="text-slate-400 text-lg leading-relaxed font-light">{moduleData.desc}</p>
               </div>
