...
export const docsTranslations: Record<string, DocsLanguageDict> = {
  'en': {
    ...
    LIMS: {
      ...
      sections: [
        { type: 'steps', subtitle: 'Registration & Collection', items: ['Verify Donor via National ID (CCCD).', 'Execute Medical Screening & Survey.', 'Generate and apply ISBT-128 barcodes.', 'Digital binding of collection bag to donor record.', 'Real-time phlebotomy volume monitoring.'] },
        { type: 'alert', alertType: 'info', content: 'Rare Donor Flag: If a registered donor has a rare phenotype, the LIMS node instantly triggers a notification to the National Rare Registry.' }
      ]
    },
    LAB: {
    ...
    MDM: {
      ...
      sections: [
        { type: 'text', subtitle: 'Network Governance', content: 'Used to establish organizational hierarchies (Hospitals, Centers, Hubs) and provision role-based access for tactical accounts.' },
        { type: 'list', subtitle: 'Admin Responsibilities', items: ['Manage ISBT-128 Product Catalogs.', 'Configure Crossmatch efficiency rules.', 'Manage User Accounts & Passwords.', 'Audit Log Supervision.'] }
      ]
    }
  },
  'zh-TW': {
    ...
    LIMS: {
      ...
      sections: [
        { type: 'steps', subtitle: '註冊與採血流程', items: ['使用國民身分證 (CCCD) 驗證捐血者。', '執行醫學篩檢與健康問卷。', '生成並張貼 ISBT-128 國際標準條碼。', '數位化綁定採血袋與捐血者紀錄。', '實時監控採集容量。'] },
        { type: 'alert', alertType: 'info', content: '稀有血型標記：若註冊者為罕見表型，系統會立即觸發通知至國家稀有血型名冊。' }
      ]
    },
    LAB: {
    ...
    MDM: {
      ...
      sections: [
        { type: 'text', subtitle: '網格治理', content: '用於建立組織層級（醫院、中心、中轉站）並為作戰帳號配置角色存取權限。' },
        { type: 'list', subtitle: '管理職責', items: ['維護 ISBT-128 產品目錄。', '配置交叉配血效率規則。', '管理使用者帳號與密碼。', '系統稽核日誌監督。'] }
      ]
    }
  },
  'vi': {
    ...
    LIMS: {
      ...
      sections: [
        { type: 'steps', subtitle: 'Đăng ký & Lấy máu', items: ['Xác minh qua CCCD.', 'Khám sàng lọc y tế.', 'Tạo mã vạch ISBT-128.', 'Liên kết túi máu với hồ sơ.', 'Theo dõi thể tích lấy máu.'] }
      ]
    },
    LAB: {
    ...
    MDM: {
      ...
      sections: [
        { type: 'list', subtitle: 'Trách nhiệm Quản trị', items: ['Quản lý danh mục ISBT-128.', 'Cấu hình quy tắc hiệu quả.', 'Quản lý tài khoản & mật khẩu.', 'Giám sát nhật ký kiểm toán.'] }
      ]
    }
  }
};
