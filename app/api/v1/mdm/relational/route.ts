...
function superuserUnauthorizedResponse(request: Request) {
  return apiErrorResponse({
    request,
    code: 'SUPERUSER_SESSION_INVALID',
    message: 'Access Denied: Missing or expired Superuser session token.',
    status: 401,
  });
}

function constraintViolationResponse(request: Request, code: string, message: string) {
  return apiErrorResponse({
    request,
    code,
    message,
    ...
    details: {
      severity: 'HardStop',
      actionRequired: 'Correct the relational integrity issue before retrying this MDM mutation.',
    },
  });
}
...
export async function GET(request: Request) {
  try {
    ...
    if (!isValidToken(authHeader)) {
      return superuserUnauthorizedResponse(request);
    }
    ...
    return internalErrorResponse(request, error, 'MDM_RELATIONAL_READ_FAILED');
  }
}
  ...
  try {
    ...
    if (action === 'auth') {
      ...
      if (!username || !password) {
        return apiErrorResponse({
          request,
          code: 'SUPERUSER_AUTH_INVALID_PAYLOAD',
      ...
      // Emergency fallback credentials for non-production/demo environments.
      ...
      if (!user) {
        return apiErrorResponse({
          request,
          code: 'SUPERUSER_AUTH_FAILED',
          message: 'User not found or database unreachable',
          status: 401,
        });
      }
      ...
      if (user.role !== 'Admin') {
        return apiErrorResponse({
          request,
          code: 'SUPERUSER_ADMIN_REQUIRED',
          message: 'Access Denied: Only users with the highest administrative role (Admin) can access the Superuser Terminal.',
          status: 403,
        });
      }
    ...
    if (action === 'mutation') {
      const authz = authorizeApiRole({
        ...
        body,
        allowedRoles: ['Admin'],
        action: 'MDM_RELATIONAL_MUTATION',
      });
      if (!authz.allowed) {
        return NextResponse.json(rbacErrorBody(authz), { status: 403 });
      }
      ...
      if (!type || !table || !rowId) {
        return apiErrorResponse({
          request,
          code: 'MDM_MUTATION_INVALID_PAYLOAD',
          message: 'Missing mutation parameters',
          status: 400,
        });
      }
      ...
      if (!targetDb) {
        return apiErrorResponse({
          request,
          code: 'MDM_TABLE_UNSUPPORTED',
          message: `Table '${table}' not supported`,
          status: 400,
        });
      }
      ...
      if (type === 'insert') {
        ...
        if (idExists) {
          return constraintViolationResponse(request, 'PRIMARY_KEY_VIOLATION', `Primary key violation: Record with ID '${rowId}' already exists in table '${table}'.`);
        }
        ...
        if (table === 'users' && data.orgId) {
          ...
          if (!orgExists) {
            return constraintViolationResponse(request, 'FOREIGN_KEY_VIOLATION', `Foreign Key Violation: Organization ID '${data.orgId}' does not exist in 'organizations'.`);
          }
        }
        ...
        if (table === 'questionnaires' && data.donorId) {
          ...
          if (!donorExists) {
            return constraintViolationResponse(request, 'FOREIGN_KEY_VIOLATION', `Foreign Key Violation: Donor ID '${data.donorId}' does not exist in 'donors'.`);
          }
        }
        ...
        if (table === 'donations') {
          ...
          if (data.questionnaireId) {
            ...
            if (!questExists) {
              return constraintViolationResponse(request, 'FOREIGN_KEY_VIOLATION', `Foreign Key Violation: Questionnaire ID '${data.questionnaireId}' does not exist in 'questionnaires'.`);
            }
          }
        }
        ...
        if (table === 'lab_tests' && data.donationId) {
          ...
          if (!donationExists) {
            return constraintViolationResponse(request, 'FOREIGN_KEY_VIOLATION', `Foreign Key Violation: Donation ID '${data.donationId}' does not exist in 'donations'.`);
          }
        }
        ...
        if (table === 'orders' && data.patientId) {
          ...
          if (!patientExists) {
            return constraintViolationResponse(request, 'FOREIGN_KEY_VIOLATION', `Foreign Key Violation: Patient ID '${data.patientId}' does not exist in 'patients'.`);
          }
        }
        ...
        if (table === 'inventory' && data.productCode) {
          ...
          if (!productExists) {
            return constraintViolationResponse(request, 'FOREIGN_KEY_VIOLATION', `Foreign Key Violation: Product Code '${data.productCode}' does not exist in 'product_catalog'.`);
          }
        }
        ...
        if (table === 'transport_jobs' && data.orderId) {
          ...
          if (!orderExists) {
            return constraintViolationResponse(request, 'FOREIGN_KEY_VIOLATION', `Foreign Key Violation: Order ID '${data.orderId}' does not exist in 'orders'.`);
          }
        }
      ...
      if (type === 'delete') {
        if (table === 'organizations') {
          ...
          if (referencingUsers.length > 0 || referencingRares.length > 0 || referencingRes.length > 0) {
            return constraintViolationResponse(request, 'DELETE_RESTRICTED', `Delete Restricted: Cannot delete organization '${rowId}'. It is referenced by active users, resources, or rare donors.`);
          }
        }
        ...
        if (table === 'donors') {
          ...
          if (referencingQuests.length > 0 || referencingDonations.length > 0) {
            return constraintViolationResponse(request, 'DELETE_RESTRICTED', `Delete Restricted: Cannot delete donor '${rowId}'. It is referenced by questionnaires or active donations.`);
          }
        }
        ...
        if (table === 'questionnaires') {
          ...
          if (referencingDonations.length > 0) {
            return constraintViolationResponse(request, 'DELETE_RESTRICTED', `Delete Restricted: Cannot delete questionnaire '${rowId}'. It is referenced by active donations.`);
          }
        }
        ...
        if (table === 'donations') {
          ...
          if (referencingTests.length > 0 || referencingComps.length > 0) {
            return constraintViolationResponse(request, 'DELETE_RESTRICTED', `Delete Restricted: Cannot delete donation '${rowId}'. It is referenced by active lab tests or components.`);
          }
        }
        ...
        if (table === 'patients') {
          ...
          if (referencingOrders.length > 0 || referencingXM.length > 0 || referencingTF.length > 0 || referencingAR.length > 0) {
            return constraintViolationResponse(request, 'DELETE_RESTRICTED', `Delete Restricted: Cannot delete patient '${rowId}'. It is referenced by active clinical orders, crossmatches, transfusions, or adverse reactions.`);
          }
        }
        ...
        if (table === 'product_catalog') {
          ...
          if (referencingInv.length > 0) {
            return constraintViolationResponse(request, 'DELETE_RESTRICTED', `Delete Restricted: Cannot delete product catalog item '${rowId}'. It is referenced in blood inventory.`);
          }
        }
        ...
        if (table === 'orders') {
          ...
          if (referencingJobs.length > 0) {
            return constraintViolationResponse(request, 'DELETE_RESTRICTED', `Delete Restricted: Cannot delete order '${rowId}'. It has active transport/delivery jobs assigned.`);
          }
        }
    ...
    return apiErrorResponse({
      request,
      code: 'MDM_RELATIONAL_ACTION_INVALID',
      message: 'Invalid relational action',
      status: 400,
    });
  } catch (error: any) {
    return internalErrorResponse(request, error, 'MDM_RELATIONAL_MUTATION_FAILED');
  }
}
